...
const ToolBoxMd = ({ setBgColor, activePanel2,setActivePanel2,DesignRef}) => {
...
const category=getScreenSize();
  ...
  return (
     <div className='  w-full flex md:hidden justify-center  relative z-9  '>


            <div className={` text-white bg-white/25 backdrop-blur-lg rounded-2xl p-[0.5rem] flex   justify-center  items-center gap-[1rem]  `} >
              <div className='flex justify-around items-center w-full relative ' >
                {SmallToolBox.map((items, index) => {

                  return <button onClick={() => setActivePanel2(items.text)} className={`  ${category!=="large"?"p-[0.5rem] px-[0.5rem]":"p-[0.5rem] px-[1rem]"}   rounded-xl transition-colors duration-300 ease-in-out w-full text-nowrap ${activePanel2 === items.text && "bg-orange-400"} cursor-pointer flex flex-col justify-center  items-center `} key={index} ><span className={` ${category!=="large"?"text-sm":"text-xl"}`}>{<items.icon />}</span><p className={`  ${category!=="large"?"text-xs":"text-sm"} `}>{items.text}</p></button>
